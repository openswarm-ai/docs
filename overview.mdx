...
description: "Skills are structured instructions that teach OpenSwarm agents how to handle specific tasks — learn what they are and why they matter."
---
...
A skill is a set of structured instructions that teach an agent how to approach a specific type of task. When a skill is active, the agent follows its guidance — producing better, more consistent results for that domain than it would on its own.

Think of skills like playbooks: a support agent with a "ticket triage" skill knows exactly how to categorize, prioritize, and respond to issues following your team's process.
...
1. **Frontmatter** — A name and description that tells the agent *when* to use the skill
...
| | Skills | MCP Tools |
|---|--------|-----------|
| **What they are** | Markdown instruction files | External tool servers |
| **What they do** | Teach the agent *how* to do something | Give the agent *the ability* to do something |
| **Example** | A skill that teaches proper Word doc formatting | A Gmail MCP tool that lets the agent send emails |
| **Requires code?** | No (optional bundled scripts) | Yes (MCP server implementation) |
| **Storage** | `~/.claude/skills/` as `.md` files | Connected MCP servers |

A skill might *reference* MCP tools in its instructions (e.g., "use the Google Sheets tool to read the data, then format it like this..."), but the skill itself is not code — it's guidance.
...
| Type | Description |
|------|-------------|
| **Local Skills** | Skills you create or install yourself — stored in `~/.claude/skills/` |
| **Registry Skills** | Community skills from the [Anthropic Skills repository](https://github.com/anthropics/skills) — browsable and installable from within OpenSwarm |
